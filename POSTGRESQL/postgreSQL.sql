...
--RESTRICCIONES
--1
ALTER TABLE ALUMNO ADD CONSTRAINT CK_DNI_ALUMNO CHECK (DNI ~* '[0-9]{8}[A-Z]');
ALTER TABLE PROFESOR ADD CONSTRAINT CK_DNI_PROFESOR CHECK (DNI REGEXP '[0-9]{8}[A-Z]');
--2
ALTER TABLE ALUMNO ADD CONSTRAINT CK_MAYOR_EDAD CHECK (date(CURRENT_DATE)-date(FECHA_NACIMIENTO) >= '6570' );

--3
ALTER TABLE PROFESOR ADD CONSTRAINT CK_SAL_SMI CHECK (SALARIO >= 1166.7);
--4
ALTER TABLE COCHE ADD CONSTRAINT CK_FECHA_COMPRA CHECK (AGE (FECHA_COMPRA, CURRENT_DATE) < '5');
--5
alter table coche drop constraint ck_fecha_compra;


--CONSULTAS

--1
SELECT NOMBRE||', '||APELLIDO1||' '||APELLIDO2 AS EJERCICIO1 FROM ALUMNO WHERE DATE(CURRENT_TIMESTAMP) - DATE(FECHA_NACIMIENTO) > '10950';
...
--9
SELECT NOMBRE||' '||APELLIDO1||' '|| coalesce(to_char(FECHA_PRACTICA,'DD-MM-YYYY'), 'No tiene practicas') FROM ALUMNO A RIGHT JOIN CONDUCE C ON A.DNI=C.DNI_ALUMNO;
